...
%Problema III
duplicate([List], 1, [List]) :- !. %stop if N = 1

duplicate([List], Num, [List|List_s]) :-   %multiply each element 
        Stop is Num - 1,
        Stop > 0,
        duplicate([List], Stop, List_s).

duplicate([Head|Tail], Num, Result) :-    %append lists of multiplied elements
      duplicate([Head], Num, List1),
      duplicate(Tail, Num, List2),
